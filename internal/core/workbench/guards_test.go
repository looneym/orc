...
func TestCanOpenWorkbench(t *testing.T) {
	tests := []struct {
		name        string
		ctx         OpenWorkbenchContext
		wantAllowed bool
		wantReason  string
	}{
		{
			name: "can open workbench when all conditions met",
			ctx: OpenWorkbenchContext{
		...
		{
			name: "cannot open non-existent workbench",
			ctx: OpenWorkbenchContext{
		...
		{
			name: "cannot open workbench when path does not exist",
			ctx: OpenWorkbenchContext{
			...
			wantAllowed: false,
			wantReason:  "workbench worktree not found - run 'orc workbench create' to materialize",
		},
		{
			name: "cannot open workbench outside TMux session",
			ctx: OpenWorkbenchContext{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := CanOpenWorkbench(tt.ctx)

			if result.Allowed != tt.wantAllowed {
				t.Errorf("CanOpenWorkbench() Allowed = %v, want %v", result.Allowed, tt.wantAllowed)
			}

			if result.Reason != tt.wantReason {
				t.Errorf("CanOpenWorkbench() Reason = %q, want %q", result.Reason, tt.wantReason)
			}
		})
	}
}
