...
func TestCanCreatePR(t *testing.T) {
	tests := []struct {
		...
		{
			name: "can create PR for in-progress shipment",
			ctx: CreatePRContext{
				ShipmentID:     "SHIP-001",
				RepoID:         "REPO-001",
				ShipmentExists: true,
		...
		{
			name: "cannot create PR for draft shipment",
			ctx: CreatePRContext{
				ShipmentID:     "SHIP-001",
				RepoID:         "REPO-001",
				ShipmentExists: true,
			...
			wantAllowed: false,
			wantReason:  "can only create PR for in-progress shipments (current status: draft)",
		},
		{
			name: "cannot create PR for closed shipment",
			ctx: CreatePRContext{
				ShipmentID:     "SHIP-001",
				RepoID:         "REPO-001",
				ShipmentExists: true,
			...
			wantAllowed: false,
			wantReason:  "can only create PR for in-progress shipments (current status: closed)",
		},
		{
